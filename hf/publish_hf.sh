...
set -euo pipefail
# Code/model licenses, teacher terms, data rights and privacy must be reviewed separately.
if [ "${BORG_ALLOW_RESEARCH_UPLOAD:-}" != "1" ]; then
  echo "UPLOAD_HELD: complete model rights/privacy review, then explicitly set BORG_ALLOW_RESEARCH_UPLOAD=1" >&2
  exit 2
fi
HF="${HF_BIN:-hf}"
